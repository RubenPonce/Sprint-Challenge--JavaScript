...
console.group('objects-arrays.js');
...
const universities = []; 
//graduates.map(element=> element.university).sort();
...
const contactInfo = [];
//graduates.map(element=> `${element.first_name} ${element.email}`);
for (let i = 0; i < graduates.length; i++) {
...
const uni = [];
//graduates.filter(element=> element.university.includes('Uni'));
for (let i = 0; i < graduates.length; i++) {
...
*/
const populationTotal = zooAnimals.reduce((accumulator,element)=> element.population+accumulator,0);
